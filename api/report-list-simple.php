...
try {
    ...
    if ($q !== '') {
        $like = '%' . $q . '%';
        $where[] = '(
            r.report_number LIKE :search_report_number
            OR r.title LIKE :search_title
            OR r.summary LIKE :search_summary
            OR r.facts LIKE :search_facts
            OR r.location LIKE :search_location
        )';
        $params['search_report_number'] = $like;
        $params['search_title'] = $like;
        $params['search_summary'] = $like;
        $params['search_facts'] = $like;
        $params['search_location'] = $like;
    }

    if ($type !== '') {
        $where[] = 'r.type_code = :type';
        $params['type'] = $type;
    }
    ...
    if ($serviceFilter !== '') {
        $where[] = 'r.service_code = :service_filter';
        $params['service_filter'] = $serviceFilter;
    }

    if ($classification !== '') {
        $where[] = 'r.classification_level = :classification';
        $params['classification'] = $classification;
    }

    if ($dateFrom !== '') {
        $where[] = 'DATE(r.occurred_at) >= :date_from';
        $params['date_from'] = $dateFrom;
    }

    if ($dateTo !== '') {
        $where[] = 'DATE(r.occurred_at) <= :date_to';
        $params['date_to'] = $dateTo;
    }

    if ($createdBy !== '') {
        $where[] = 'u.username LIKE :created_by';
    ...
    $reports = array_values(array_filter(
        $stmt->fetchAll(),
        static fn (array $report): bool => canUserAccessReport($GLOBALS['user'], $report, $GLOBALS['pdo'])
    ));
