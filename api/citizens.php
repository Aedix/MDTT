...
function boolValue(array $data, string $key): int
{
    return !empty($data[$key]) && $data[$key] !== '0' && $data[$key] !== 'false' ? 1 : 0;
}
...
try {
    ...
    if ($method === 'GET' && $action === 'get') {
        $id = (int) ($_GET['id'] ?? 0);
        if ($id <= 0) jsonResponse(['success' => false, 'message' => 'Citoyen invalide.'], 400);

        $citizen = getCitizen($pdo, $id);
        if (!$citizen) jsonResponse(['success' => false, 'message' => 'Citoyen introuvable.'], 404);
        ...
        $vehiclesStatement = $pdo->prepare(
            'SELECT v.*, creator.username AS created_by_username, updater.username AS updated_by_username
             ...
             ORDER BY v.model ASC, v.category ASC, v.color ASC, v.plate ASC'
        );
    ...
    if ($method === 'POST' && $action === 'save') {
        ...
        if (!$firstName || !$lastName) jsonResponse(['success' => false, 'message' => 'Nom et prénom obligatoires.'], 400);
        ...
        $payload = [
            ...
            'health_status' => in_array(($data['health_status'] ?? 'alive'), ['alive', 'deceased'], true) ? $data['health_status'] : 'alive',
            'has_driver_license' => boolValue($data, 'has_driver_license'),
            'has_weapon_license' => boolValue($data, 'has_weapon_license'),
            'updated_by' => (int) $user['id'],
        ];

        if ($id > 0) {
            if (!getCitizen($pdo, $id)) jsonResponse(['success' => false, 'message' => 'Citoyen introuvable.'], 404);
            $payload['id'] = $id;
            $statement = $pdo->prepare(
                'UPDATE citizens SET
                    first_name = :first_name, last_name = :last_name, birth_date = :birth_date, phone = :phone, address = :address, job = :job,
                    hair_color = :hair_color, eye_color = :eye_color, height_cm = :height_cm, physical_details = :physical_details,
                    affiliation = :affiliation, known_organization = :known_organization, known_criminal_group = :known_criminal_group, special_status = :special_status,
                    notes = :notes, photo_path = :photo_path, health_status = :health_status, has_driver_license = :has_driver_license, has_weapon_license = :has_weapon_license,
            ...
            $statement = $pdo->prepare(
                'INSERT INTO citizens
                 (first_name, last_name, birth_date, phone, address, job, hair_color, eye_color, height_cm, physical_details, affiliation, known_organization, known_criminal_group, special_status, notes, photo_path, health_status, has_driver_license, has_weapon_license, created_by, updated_by)
                 VALUES
                 (:first_name, :last_name, :birth_date, :phone, :address, :job, :hair_color, :eye_color, :height_cm, :physical_details, :affiliation, :known_organization, :known_criminal_group, :special_status, :notes, :photo_path, :health_status, :has_driver_license, :has_weapon_license, :created_by, :updated_by)'
            );
            $statement->execute($payload);
    ...
    if ($method === 'POST' && $action === 'save_vehicle') {
        ...
        if ($citizenId <= 0 || $plate === '') jsonResponse(['success' => false, 'message' => 'Citoyen et plaque obligatoires.'], 400);
        if (!getCitizen($pdo, $citizenId)) jsonResponse(['success' => false, 'message' => 'Citoyen introuvable.'], 404);
        ...
        try {
            if ($id > 0) {
                $payload['id'] = $id;
                $statement = $pdo->prepare(
                    'UPDATE citizen_vehicles SET citizen_id = :citizen_id, plate = :plate, model = :model, category = :category, color = :color, registration_status = :registration_status, photo_path = :photo_path, notes = :notes, updated_by = :updated_by WHERE id = :id'
                );
                $statement->execute($payload);
                ...
                $payload['created_by'] = (int) $user['id'];
                $statement = $pdo->prepare(
                    'INSERT INTO citizen_vehicles (citizen_id, plate, model, category, color, registration_status, photo_path, notes, created_by, updated_by)
                     VALUES (:citizen_id, :plate, :model, :category, :color, :registration_status, :photo_path, :notes, :created_by, :updated_by)'
                );
                $statement->execute($payload);
                $id = (int) $pdo->lastInsertId();
    ...
    if ($method === 'POST' && $action === 'save_record') {
        ...
        if ($citizenId <= 0 || !$offenseType) jsonResponse(['success' => false, 'message' => 'Citoyen et type d’infraction obligatoires.'], 400);
        if (!getCitizen($pdo, $citizenId)) jsonResponse(['success' => false, 'message' => 'Citoyen introuvable.'], 404);
        ...
        if ($id > 0) {
            $payload['id'] = $id;
            $statement = $pdo->prepare('UPDATE criminal_records SET citizen_id = :citizen_id, offense_date = :offense_date, offense_type = :offense_type, description = :description, case_status = :case_status, sanction = :sanction, notes = :notes, updated_by = :updated_by WHERE id = :id');
            $statement->execute($payload);
            addCitizenLog($pdo, $citizenId, 'criminal_record', $id, 'update', $payload, (int) $user['id']);
        } else {
            $payload['created_by'] = (int) $user['id'];
            $statement = $pdo->prepare('INSERT INTO criminal_records (citizen_id, offense_date, offense_type, description, case_status, sanction, notes, created_by, updated_by) VALUES (:citizen_id, :offense_date, :offense_type, :description, :case_status, :sanction, :notes, :created_by, :updated_by)');
            $statement->execute($payload);
            $id = (int) $pdo->lastInsertId();
    ...
    if ($method === 'POST' && in_array($action, ['delete_vehicle', 'delete_record'], true)) {
        if (!canDeleteCitizenData($user)) jsonResponse(['success' => false, 'message' => 'Droits insuffisants pour supprimer.'], 403);
        $data = bodyData();
        $id = (int) ($data['id'] ?? 0);
