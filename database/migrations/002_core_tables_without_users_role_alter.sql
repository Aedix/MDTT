-- Migration safe MDT.
-- À utiliser si users.role existe déjà.
-- Ce fichier ne modifie pas la table users.
...
CREATE TABLE IF NOT EXISTS ranks (
  ...
  CONSTRAINT fk_ranks_service FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
ON DUPLICATE KEY UPDATE label = VALUES(label), power_level = VALUES(power_level), is_system = VALUES(is_system), is_active = VALUES(is_active);
...
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('mdt.access') WHERE r.code IN ('user', 'officer', 'senior');

INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('mdt.access', 'panel.access', 'accounts.view', 'accounts.activate', 'accounts.deactivate') WHERE r.code IN ('sergeant', 'lieutenant');

INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('mdt.access', 'panel.access', 'service.full_access', 'accounts.view', 'accounts.activate', 'accounts.deactivate', 'accounts.change_rank', 'ranks.view', 'ranks.create', 'ranks.rename', 'ranks.move', 'ranks.delete') WHERE r.code = 'chief';

INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('mdt.access', 'panel.access', 'service.full_access', 'accounts.view', 'accounts.activate', 'accounts.deactivate', 'accounts.change_rank', 'accounts.reset_password', 'ranks.view', 'ranks.create', 'ranks.rename', 'ranks.move', 'ranks.delete', 'roles.view', 'roles.assign', 'permissions.view') WHERE r.code = 'admin';

INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r JOIN permissions p ON p.code IN ('mdt.access', 'panel.access', 'service.full_access', 'accounts.view', 'accounts.activate', 'accounts.deactivate', 'accounts.change_rank', 'accounts.reset_password', 'ranks.view', 'ranks.create', 'ranks.rename', 'ranks.move', 'ranks.delete', 'roles.view', 'roles.assign', 'permissions.view', 'permissions.manage', 'services.view', 'services.manage') WHERE r.code = 'super_admin';
...
INSERT INTO ranks (service_id, code, name, level, sort_order, is_command, is_active)
SELECT s.id, 'chief', 'Chief', 80, 80, 1, 1 FROM services s WHERE s.code IN ('FIB', 'LSPD', 'SAMS')
ON DUPLICATE KEY UPDATE name = VALUES(name), level = VALUES(level), sort_order = VALUES(sort_order), is_command = VALUES(is_command), is_active = VALUES(is_active);
